...
Describe 'Invoke-IPC' {
    It 'throws when multiple device selectors are provided' {
        { Invoke-IPC -Action ListDevices -DeviceName 'test' -AllDevices } | Should -Throw '*only one*'
    }

    It 'throws when no device selector is provided for non-ListDevices action' {
        { Invoke-IPC -Action SoftwareInventory } | Should -Throw '*Specify*'
    }

    It 'validates Action parameter values' {
        { Invoke-IPC -Action 'InvalidAction' -DeviceName 'test' } | Should -Throw
    }

    It 'accepts valid Action parameter values' {
        $cmd = Get-Command Invoke-IPC
